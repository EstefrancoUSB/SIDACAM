...
Frec_Corte1 = 300;
N_Frec = 35;
Step = 50 ;         %Delta del banco de filtros, desviacion estandar
...
for i=1:N_Frec
    ...
    Orden_Filtro = 8;
    [~,position]=min(abs(Frecuencias-Step));  % Busca la posiciï¿½n del vector "Frecuencias"       
                                                %mas cercana a el Step
    Frec_Corte1 = Frec_Max_B(i)-Frecuencias(position);
    Frec_Corte2 = Frec_Max_B(i)+Frecuencias(position);
    % Diseï¿½ando el filtro pasabando
    Param_Filtro = fdesign.bandpass('N,F3dB1,F3dB2',...
        Orden_Filtro,Frec_Corte1,Frec_Corte2,Frec_Muestreo);
    Filtro = design(Param_Filtro,'butter');
    % Aplicando filtros al Ruido de fondo y a la seï¿½al de blando
    ...
    %Aplicando Ecuaciï¿½n de Sonar Pasivo
