...
% Cargando se�al a analizar en S_Blanco. 
...
for i=1:N_Frec
    ...
    Orden_Filtro = 8;
    [~,position]=min(abs(Frecuencias-Step));  % Busca la posici�n del vector "Frecuencias"       
                                                %mas cercana a el Step
    Frec_Corte1 = Frec_Max_B(i)-Frecuencias(position);
    Frec_Corte2 = Frec_Max_B(i)+Frecuencias(position);
    % Dise�ando el filtro pasabando
    Param_Filtro = fdesign.bandpass('N,F3dB1,F3dB2',...
        Orden_Filtro,Frec_Corte1,Frec_Corte2,Frec_Muestreo);
    Filtro = design(Param_Filtro,'butter');
    % Aplicando filtros al Ruido de fondo y a la se�al de blando
    ...
    %Aplicando Ecuaci�n de Sonar Pasivo
